...
$carnet=$_GET['txDev'];
$_SESSION["txDev"]=$carnet;
if(isset($carnet))
{
$selectNombre="SELECT nombreAlumno FROM alumnos WHERE carnet='".$carnet."';";
$selectNombreScript=mysql_query($selectNombre, $conexion);
$prestamosCount="SELECT * FROM prestamo WHERE carnet='".$carnet."' AND Estado!='En Canasta';";
$prestamosCountScript=mysql_query($prestamosCount, $conexion);
$numeroDePrestamos = mysql_num_rows($prestamosCountScript);
$libroCanasta="SELECT * FROM prestamo p INNER JOIN libros l ON (l.idLibro = p.idLibro) WHERE p.carnet='".$carnet."';";
...
{
	$cc1=substr($carnet, 0,2);
	$cc2=substr($carnet, 2,4);
	$cc3=substr($carnet, 6,8);
?>
	<center>El Usuario: <b><?php echo($cc1."-".$cc2."-".$cc3);?></b> No realiz&oacute; Prestamos</center>
<?php
}
else
{
...
<?php
	...
	{
	echo("<center><table border='0'><tr><td colspan='2' align='center'><a class='tit'>Nombre del Alumno</a></td></tr><tr><td colspan='2' align='center'><a class='nom'><b>".$al["nombreAlumno"]."</b></a></td></tr>");
	echo("<tr><td><a class='tit'>Prestamos Realizados:</a></td><td><a class='nom'><b><i>".$numeroDePrestamos." Libro/s<i></b></a></td></tr></table></center>");
	?>
	<br>
	<center>
		...
		{
		$can=mysql_fetch_array($libroCanastaScript, MYSQL_ASSOC);
		...
		<input type="submit" name="sbEnviar" value="Devolver Libro" disabled="disabled" class="botonS">
