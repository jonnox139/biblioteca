...
if($numerosMoras!=0)
{
	...
	<center>
	<a class="titulo"><b>UNIVERSIDAD TECNOL&Oacute;GICA DE EL SALVADOR</b></a><br>
	Sistema Bibliotecario UTEC<br>
	Moras Existentes <i><?php echo($diaActual." de ".$mesActual." del ".$annioActual);?></i>:
	<?php
	$mostrarMoras="SELECT m.carnet, m.cantidad, m.fechaMora, a.nombreAlumno FROM alumnos a INNER JOIN mora m ON(a.carnet = m.carnet) WHERE estadoMora='Activa';";
	$mostrarMorasScript=mysql_query($mostrarMoras, $conexion);
	echo("<a class='numeros'>->".$numerosMoras." Alumnos<-<br></a>");
	echo("<table border='2'>");
	echo("<tr><th>Carnet</th><th>Nombre del Alumno</th><th>Precio</th><th>Fecha de la Mora</th></tr>");
	while($mr=mysql_fetch_array($mostrarMorasScript, MYSQL_ASSOC))
	{
		$cc1=substr($mr["carnet"], 0,2);
		$cc2=substr($mr["carnet"], 2,4);
		$cc3=substr($mr["carnet"], 6,8);
		$annioMora=substr($mr["fechaMora"],0,4);
		$mesMora=substr($mr["fechaMora"],5,2);
		$diaMora=substr($mr["fechaMora"],8);
		switch($mesMora)
		{	
			case 1:
			{
	...
	<td><?php echo($mr["nombreAlumno"]); ?></td>
...
<SCRIPT language="javascript">
		...
		function mensaje()
		{
			alert("No hay Alumnos con Moras");
...
<style type="text/css">
	.titulo{ font-family:verdana; font-size:15px; }
