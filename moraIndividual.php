...
<?php
$mostrarMoras="SELECT m.carnet, m.cantidad, m.fechaMora, a.nombreAlumno FROM alumnos a INNER JOIN mora m ON(a.carnet = m.carnet) WHERE estadoMora='Activa' AND m.carnet='".$carnet."';";
$mostrarMorasScript=mysql_query($mostrarMoras, $conexion);
echo("<table border='1' align='center'>");
while($mrInd=mysql_fetch_array($mostrarMorasScript, MYSQL_ASSOC))
{
	$cc1=substr($mrInd["carnet"], 0,2);
	$cc2=substr($mrInd["carnet"], 2,4);
	$cc3=substr($mrInd["carnet"], 6,8);
	echo("<tr><td><b>Carnet:</b></td><td align='center'>".$cc1."-".$cc2."-".$cc3. "</td></tr>");
	echo("<tr><td><b>Nombre:</b></td><td>".$mrInd["nombreAlumno"]. "</td></tr>");
	echo("<tr><td><b>Total a pagar:</b></td><td align='center'>=><i><b><font color='red'>$ ".$mrInd["cantidad"]. "</font></b></i><=</td></tr>");
	$annioMora=substr($mrInd["fechaMora"],0,4);
	$mesMora=substr($mrInd["fechaMora"],5,2);
	$diaMora=substr($mrInd["fechaMora"],8);
	switch($mesMora)
	{	
		case 1:
		{
